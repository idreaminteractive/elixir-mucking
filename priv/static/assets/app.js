...
`);
  ...
  // ../deps/live_ex_webrtc/assets/publisher.js
  function createPublisherHook(iceServers2 = []) {
    return {
      async mounted() {
        const view = this;
      ...
      async findDevices(view) {
        ...
        devices.forEach((device) => {
          if (device.kind === "videoinput") {
            view.videoDevices.options[view.videoDevices.options.length] = new Option(device.label, device.deviceId);
          } else if (device.kind === "audioinput") {
            view.audioDevices.options[view.audioDevices.options.length] = new Option(device.label, device.deviceId);
          }
        });
        view.closeStream(view);
      },
      closeStream(view) {
        if (view.localStream != void 0) {
          console.log(`Closing stream with id: ${view.localStream.id}`);
          view.localStream.getTracks().forEach((track) => track.stop());
          view.localStream = void 0;
        }
      },
      ...
      async startStreaming(view) {
        ...
        view.pc = new RTCPeerConnection({ iceServers: iceServers2 });
        view.pc.onconnectionstatechange = () => {
          if (view.pc.connectionState === "connected") {
            view.startTime = /* @__PURE__ */ new Date();
            ...
            view.statsIntervalId = setInterval(async function() {
              if (!view.pc) {
                ...
                view.statsIntervalId = void 0;
                return;
              }
              view.time.innerText = view.toHHMMSS(/* @__PURE__ */ new Date() - view.startTime);
              const stats = await view.pc.getStats(null);
              let bitrate;
              stats.forEach((report) => {
                if (report.type === "outbound-rtp" && report.kind === "video") {
                  if (!view.lastVideoReport) {
                    bitrate = report.bytesSent * 8 / 1e3;
                  } else {
                    const timeDiff = (report.timestamp - view.lastVideoReport.timestamp) / 1e3;
                    if (timeDiff == 0) {
                      bitrate = 0;
                    } else {
                  ...
                  view.videoBitrate.innerText = (bitrate / 1e3).toFixed();
                  view.lastVideoReport = report;
                } else if (report.type === "outbound-rtp" && report.kind === "audio") {
                  if (!view.lastAudioReport) {
                    bitrate = report.bytesSent;
                  } else {
                    const timeDiff = (report.timestamp - view.lastAudioReport.timestamp) / 1e3;
                    if (timeDiff == 0) {
                      bitrate = 0;
                    } else {
                  ...
                  view.audioBitrate.innerText = (bitrate / 1e3).toFixed();
              ...
              if (!view.lastAudioReport || !view.lastVideoReport) {
                ...
                const packetsSent = view.lastVideoReport.packetsSent + view.lastAudioReport.packetsSent;
                const rtxPacketsSent = view.lastVideoReport.retransmittedPacketsSent + view.lastAudioReport.retransmittedPacketsSent;
                const nackReceived = view.lastVideoReport.nackCount + view.lastAudioReport.nackCount;
                if (nackReceived == 0) {
                  view.packetLoss.innerText = 0;
                } else {
                  view.packetLoss.innerText = (nackReceived / (packetsSent - rtxPacketsSent) * 100).toFixed();
                }
              }
            }, 1e3);
        ...
        view.pc.getSenders().filter((sender) => sender.track.kind === "video").forEach(async (sender) => {
      ...
      resetStats(view) {
        view.startTime = void 0;
        view.lastAudioReport = void 0;
        view.lastVideoReport = void 0;
      ...
      toHHMMSS(milliseconds) {
        let hours = Math.floor(milliseconds / (1e3 * 60 * 60));
        let minutes = Math.floor(milliseconds % (1e3 * 60 * 60) / (1e3 * 60));
        let seconds = Math.floor(milliseconds % (1e3 * 60) / 1e3);
  ...
  // ../deps/live_ex_webrtc/assets/player.js
  function createPlayerHook(iceServers2 = []) {
    return {
      async mounted() {
        this.pc = new RTCPeerConnection({ iceServers: iceServers2 });
  ...
  Hooks2.Publisher = createPublisherHook(iceServers);
  Hooks2.Player = createPlayerHook(iceServers);
